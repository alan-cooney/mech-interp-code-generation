...
def get_corner(tens, k=3):
    sl = slice(0, k)
    return tens[(tuple([sl for i in range(len(tens.shape))]))]
...
cfg['tokens_per_step'] = (
    cfg['batch_size']*cfg['n_ctx']*cfg['batches_per_step'])
...
def scatter(x, y, **kwargs):
    px.scatter(x=to_numpy(x, flat=True), y=to_numpy(
        y, flat=True), **kwargs).show()
...
def lines(lines_list, x=None, mode='lines', labels=None, xaxis='', yaxis='', title='', log_y=False, hover=None, **kwargs):
    ...
    for c, line in enumerate(lines_list):
        ...
        if labels is not None:
            ...
            label = c
        fig.add_trace(go.Scatter(x=x, y=line, mode=mode,
                      name=label, hovertext=hover, **kwargs))
    if log_y:
...
def animate_lines(lines_list, snapshot_index=None, snapshot='snapshot', hover=None, xaxis='x', yaxis='y', **kwargs):
    ...
    px.line(df, x=xaxis, y=yaxis, animation_frame=snapshot, range_y=[
            lines_list.min(), lines_list.max()], hover_name=hover, **kwargs).show()
...
def animate_multi_lines(lines_list, y_index=None, snapshot_index=None, snapshot='snapshot', hover=None, swap_y_animate=False, **kwargs):
    ...
    px.line(df, x='x', y=y_index, animation_frame=snapshot, range_y=[
            lines_list.min(), lines_list.max()], hover_name=hover, **kwargs).show()
...
def animate_scatter(lines_list, snapshot_index=None, snapshot='snapshot', hover=None, yaxis='y', xaxis='x', color=None, color_name='color', **kwargs):
    ...
    if len(color.shape) == 1:
        color = einops.repeat(color, 'x -> snapshot x',
                              snapshot=lines_list.shape[0])
    print(lines_list.shape)
    rows = []
    for i in range(lines_list.shape[0]):
        for j in range(lines_list.shape[2]):
            rows.append([lines_list[i, 0, j].item(), lines_list[i,
                        1, j].item(), snapshot_index[i], color[i, j]])
    ...
    px.scatter(df, x=xaxis, y=yaxis, animation_frame=snapshot, range_x=[lines_list[:, 0].min(), lines_list[:, 0].max(
    )], range_y=[lines_list[:, 1].min(), lines_list[:, 1].max()], hover_name=hover, color=color_name, **kwargs).show()
# %%
...
class FactoredEmbed(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.W_E = nn.Parameter(torch.empty(
            self.cfg['factor_size'], self.cfg['d_vocab']))
        self.W_E_factor = nn.Parameter(torch.empty(
            self.cfg['d_model'], self.cfg['factor_size']))
        nn.init.kaiming_uniform_(self.W_E, a=np.sqrt(5))
...
class Unembed(nn.Module):
    ...
    def forward(self, tokens):
        # [batch, pos, d_vocab]
        return torch.einsum('vm,bpm->bpv', self.W_U, tokens)
...
class FactoredUnembed(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.W_U = nn.Parameter(torch.empty(
            self.cfg['d_vocab'], self.cfg['factor_size']))
        self.W_U_factor = nn.Parameter(torch.empty(
            self.cfg['factor_size'], self.cfg['d_model']))
        nn.init.kaiming_uniform_(self.W_U, a=np.sqrt(5))
    ...
    def forward(self, tokens):
        ...
        # B acts as a tensor of indices into the second dimension (so >=0 and <b)
        # [batch, pos, d_vocab]
        return torch.einsum('fm,vf,bpm->bpv', self.W_U_factor, self.W_U, tokens)
...
class PosEmbed(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.W_pos = nn.Parameter(torch.empty(
            self.cfg['d_model'], self.cfg['n_ctx']))
...
class Attention(nn.Module):
    ...
    def __init__(self, cfg, attn_type='global'):
        ...
        self.W_O = nn.Parameter(torch.empty(
            self.cfg['n_heads'], self.cfg['d_model'], self.cfg['d_head']))
        ...
        causal_mask = torch.tril(torch.ones(
            (self.cfg['n_ctx'], self.cfg['n_ctx'])).bool())
    ...
    def forward(self, x, pos_embed):
        if not cfg['use_pos_resid']:
            ...
            q = self.hook_q(torch.einsum('ihm,bpm->bpih',
                            self.W_Q, attn_input)+self.b_Q)
            # [batch, pos, head_index, d_head]
            k = self.hook_k(torch.einsum('ihm,bpm->bpih',
                            self.W_K, attn_input)+self.b_K)
        else:
            # [batch, pos, head_index, d_head]
            q = self.hook_q(torch.einsum(
                'ihm,bpm->bpih', self.W_Q, x)+self.b_Q)
            # [batch, pos, head_index, d_head]
            k = self.hook_k(torch.einsum(
                'ihm,bpm->bpih', self.W_K, x)+self.b_K)

        # [batch, pos, head_index, d_head]
        v = self.hook_v(torch.einsum('ihm,bpm->bpih', self.W_V, x)+self.b_V)
        # [batch, head_index, query_pos, key_pos]
        attn_scores = torch.einsum('bpih,bqih->bipq', q, k)/self.attn_scale
        attn_scores = self.hook_attn_scores(self.apply_causal_mask(
            attn_scores))  # [batch, head_index, query_pos, key_pos]
        # [batch, head_index, query_pos, key_pos]
        attn_matrix = self.hook_attn(F.softmax(attn_scores, dim=-1))
        # [batch, pos, head_index, d_head]
        z = self.hook_z(torch.einsum('bpih,biqp->bqih', v, attn_matrix))
        ...
        if cfg['use_attn_result']:
            ...
            out = (torch.einsum('imh,bqih->bqm', self.W_O, z) +
                   self.b_O)  # [batch, pos, head_index, d_model]
        return out
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, pos_embed):
        ...
        if self.cfg['normalization'] is not None:
            attn_out = self.hook_attn_out(self.attn(self.norm1(
                resid_pre), pos_embed))  # [batch, pos, d_model]
        else:
            attn_out = self.hook_attn_out(
                self.attn(resid_pre, pos_embed))  # [batch, pos, d_model]
...
class AttnOnlyBlock(nn.Module):
    ...
    def forward(self, x, pos_embed):
        resid_pre = self.hook_resid_pre(x)  # [batch, pos, d_model]
        attn_out = self.hook_attn_out(
            self.attn(x, pos_embed))  # [batch, pos, d_model]
        resid_post = self.hook_resid_post(
            resid_pre + attn_out)  # [batch, pos, d_model]
        return resid_post
...
class AttnOnlyTransformer(HookedRootModule):
    def __init__(self, cfg, tokenizer):
        ...
        self.blocks = nn.ModuleList([AttnOnlyBlock(
            self.cfg, block_index) for block_index in range(self.cfg['n_layers'])])
        self.unembed = Unembed(self.cfg)
...
def tokenize(examples):
    ...
    tokens = einops.rearrange(
        tokens, '(batch_size seq) -> batch_size seq', batch_size=curr_batch_size, seq=seq_len-1)
...
class MaxActStore():
    ...
    def __init__(self, length, name="", top_k=10, device='cuda', log=True, log_every=200, dtype=DTYPE):
        ...
        self.max_acts = -torch.inf * \
            torch.ones(length, top_k).to(device).to(dtype)
        print(self.max_acts)
    ...
    def update(self, x: torch.Tensor):
        ...
        if self.log and (self.counter % self.log_every == 0):
            wandb.log({f'{self.name}_max_act': x.max().item(), f'{self.name}_new_updates': new_updates,
                      f'{self.name}_total_updates': self.total_updates}, step=self.counter)
    ...
    def batch_update(self, x: torch.Tensor):
        ...
        if self.log and ((self.counter//batch_size) % self.log_every == 0):
            wandb.log({f'{self.name}_max_act': x.max().item(), f'{self.name}_new_updates': new_updates, f'{self.name}_total_updates': self.total_updates,
                      f'{self.name}_num_steps': self.num_steps, 'elapsed': time.time()-self.start_time}, step=self.counter)
            self.num_steps = 0
...
def save_stores_dict(stores_dict, file_name, do_wandb=True):
    print(
        f"Saving stores dict to {file_name}. Available stores are {stores_dict.keys()}")
...
def vis_activations(str_tokens, activations, name="", incl_bos=True):
    ...
    if incl_bos:
        pysvelte.TextSingle(
            tokens=str_tokens, activations=activations[:], neuron_name=name).show()
    else:
        pysvelte.TextSingle(
            tokens=str_tokens[1:], activations=activations[1:], neuron_name=name).show()
...
def print_neuron_logits(neuron_index, top_k=5):
    ...
    for i in range(top_k):
        l.append(
            f"|{tokenizer.decode([logit_indices[-i-1].item()], clean_up_tokenization_spaces=False)}| {logit_vec[-i-1].item():.6f}")
    l.append('...')
    for i in range(top_k):
        l.append(
            f"|{tokenizer.decode([logit_indices[top_k-i-1].item()], clean_up_tokenization_spaces=False)}| {logit_vec[top_k-i-1].item():.6f}")
    print("\n".join(l))
